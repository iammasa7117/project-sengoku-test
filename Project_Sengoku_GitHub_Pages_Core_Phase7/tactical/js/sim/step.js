(function (T) {
  ...
  T.stepSimulation=function(state){
    ...
    var playerUnits=T.getUnits(state,"player"), enemyUnits=T.getUnits(state,"enemy");
    var pBreakThreshold=Math.max(1,Math.ceil(playerUnits.length*5/7)), eBreakThreshold=Math.max(1,Math.ceil(enemyUnits.length*5/7));
    var playerCommander=playerUnits.filter(function(u){return u.isCommander;})[0];
    var enemyCommander=enemyUnits.filter(function(u){return u.isCommander;})[0];
    var playerDefeated=pBroken>=pBreakThreshold || !playerCommander || playerCommander.status==="destroyed" || activeCount(state,"player")===0;
    var enemyDefeated=eBroken>=eBreakThreshold || !enemyCommander || enemyCommander.status==="destroyed" || activeCount(state,"enemy")===0;
