...
function classList(owner) { return { add: function (name) { var list = owner.className.split(/\s+/).filter(Boolean); if (list.indexOf(name) < 0) list.push(name); owner.className = list.join(" "); }, remove: function (name) { owner.className = owner.className.split(/\s+/).filter(function (item) { return item && item !== name; }).join(" "); }, contains: function (name) { return owner.className.split(/\s+/).indexOf(name) >= 0; } }; }
function element(id) {
  if (elements[id]) return elements[id];
  var item = { id: id, innerHTML: "", textContent: "", value: "", className: id === "modalBackdrop" ? "modal-backdrop hidden" : "", dataset: {}, children: [], attributes: {}, disabled: false,
    appendChild: function (child) { this.children.push(child); }, setAttribute: function (name, value) { this.attributes[name] = String(value); }, focus: function () { document.activeElement = this; },
    querySelector: function (selector) { if (id === "modalBackdrop" && selector === ".modal") return element("modalDialog"); if (/h1|h2|h3/.test(selector)) return element("modalHeading"); if (/button|select|input|textarea/.test(selector)) return element("focusable"); return null; }, querySelectorAll: function () { return []; }
  };
  item.classList = classList(item); elements[id] = item; return item;
}
global.window = global; global.addEventListener = function () {};
global.document = { title: "", activeElement: null, body: element("body"), getElementById: element, createElement: function () { return element("created-" + Math.random()); }, createElementNS: function () { return element("svg-" + Math.random()); } };
