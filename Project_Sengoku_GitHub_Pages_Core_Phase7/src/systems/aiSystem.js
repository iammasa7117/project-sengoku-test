(function (S) {
  ...
  A.takeAction = function (state, factionId, options) {
    ...
    var reinforced = A.reinforce(state, factionId); if (reinforced.ok) return reinforced;
    ...
    var attacked = A.attack(state, factionId, { avoidFactionId: options.avoidFactionId, random: options.random }); if (attacked.ok) return attacked;
  ...
  A.runSeason = function (state, options) {
    ...
    Object.keys(state.factions).filter(function (id) { return state.factions[id].alive && (options.allFactions || id !== state.campaign.playerFactionId); }).forEach(function (factionId) {
      ...
      for (var i = 0; i < count && !state.campaign.gameOver; i += 1) {
        var result = A.takeAction(state, factionId, { random: rng, strategicSimulation: Boolean(options.allFactions), avoidFactionId: playerAttackCount >= playerAttackLimit ? state.campaign.playerFactionId : null });
        if (result.ok) {
