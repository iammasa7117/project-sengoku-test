(function (S) {
  ...
  function finishSeason(state, options, rng) {
    ...
    if (!state.campaign.gameOver && state.settings.aiEnabled && !options.skipAI) {
      ...
      aiResult = S.Systems.AI.runSeason(state, { random: rng, allFactions: Boolean(options.allFactionsAI), simulation: Boolean(options.simulation) });
    }
  ...
  T.advance = function (state, options) {
    ...
    if (state.events.pendingTacticalBattle) {
      ...
      state.events.pendingTacticalBattle.resumeArmyState = armyPhase.stateChanges && armyPhase.stateChanges.resumeState || null;
      return { ok: economyPhase.ok && diplomacyPhase.ok && armyPhase.ok, stateChanges: { interruptedByTactical: true, economyPhase: economyPhase.stateChanges, diplomacyPhase: diplomacyPhase.stateChanges, armyActions: armyPhase.stateChanges.actions, aiDiplomacyActions: [], aiActions: [], season: state.campaign.season, year: state.campaign.year }, messages: [state.events.pendingTacticalBattle && state.events.pendingTacticalBattle.kind === "field_battle" ? "敵軍と接触しました。迎撃野戦を指揮してください。" : "進軍Armyが敵城へ到着しました。会戦を指揮してください。"], errors: (economyPhase.errors || []).concat(diplomacyPhase.errors || [], armyPhase.errors || []) };
    }
  ...
  T.resumeAfterTactical = function (state, options) {
    ...
    var rng = options.random || Math.random, armyPhase = { ok: true, stateChanges: { actions: [], resumeState: null }, errors: [] };
    if (options.armyResumeState && S.Systems.Army && S.Systems.Army.advanceSeason) {
      armyPhase = S.Systems.Army.advanceSeason(state, { random: rng, allowTactical: options.allowTactical !== false, resumeState: options.armyResumeState });
      if (state.events && state.events.pendingTacticalBattle) {
        state.events.pendingTacticalBattle.resumeSeason = true;
        state.events.pendingTacticalBattle.resumeArmyState = armyPhase.stateChanges && armyPhase.stateChanges.resumeState || null;
        return { ok: armyPhase.ok, stateChanges: { interruptedByTactical: true, armyActions: armyPhase.stateChanges.actions, season: state.campaign.season, year: state.campaign.year }, messages: [state.events.pendingTacticalBattle.kind === "field_battle" ? "別の敵軍と接触しました。野戦を指揮してください。" : "別の進軍Armyが敵城へ到着しました。会戦を指揮してください。"], errors: armyPhase.errors || [] };
      }
    }
    var finished = finishSeason(state, options, rng);
    return { ok: armyPhase.ok && finished.ok, stateChanges: { season: finished.stateChanges.season, year: finished.stateChanges.year, armyActions: armyPhase.stateChanges.actions, aiDiplomacyActions: finished.stateChanges.aiDiplomacyActions, aiActions: finished.stateChanges.aiActions }, messages: [], errors: (armyPhase.errors || []).concat(finished.errors || []) };
  };
