(function (S) {
  ...
  U.renderCouncil = function () {
    ...
    root.innerHTML = "<article class=\"proposal\"><p class=\"kicker\">" + U.escape(leader ? leader.name : "家臣") + "の提案</p><h4>" + U.escape(selected.name) + "の守りを整える</h4><p>徴兵して次の敵襲へ備えます。</p><button class=\"button secondary\" data-command=\"recruit\">提案を採用</button></article><article class=\"proposal\"><p class=\"kicker\">" + U.escape(strategist ? strategist.name : "家臣") + "の提案</p><h4>敵情を探る</h4><p>隣接敵城を偵察し、兵力を把握します。</p><button class=\"button secondary\" data-command=\"scout\">提案を採用</button></article><article class=\"proposal\"><p class=\"kicker\">" + U.escape(warrior ? warrior.name : "家臣") + "の提案</p><h4>攻勢へ出る</h4><p>隣接敵城へ侵攻し、戦線を進めます。</p><button class=\"button secondary\" data-command=\"attack\">提案を採用</button></article>";
  };
})(window.Sengoku);
